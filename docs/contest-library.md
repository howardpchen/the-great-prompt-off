...
Legacy organizer mutations (including all three simulation DELETE routes) require `X-Contest-Id` and `X-Contest-Version` from the rendered organizer console. Their complete handlers share the same transaction as the active-contest check, so a concurrent switch cannot retarget an action. Failure responses roll back rather than committing partial work. Clients/scripts written before this feature must reload context and send these headers.

Every framed organizer page reads its data and fence inside one repeatable-read, read-only database snapshot. The frame does not look up active identity again during rendering. Async Simulation Lab reads include the rendered context and reject mismatches; headerless read-only API callers retain their existing behavior. Library controls are unavailable until the selected response is resolved, and selection stays disabled throughout writes and their refreshes. Late responses cannot replace newer selections.

Participant POSTs require a nonempty UUID `contestId` and `schemaVersion` in addition to session/prompt fields. An old tab is rejected instead of submitting its prompt to a different contest. Admission and activation share one transaction advisory lock; a unique partial index independently enforces the one-active invariant.
...
- `TEST_ALLOW_MUTATIONS=true USE_REAL_LLM=false npm run test:admin-page-snapshot` runs after `test:contest-library` in that disposable database. It deterministically switches active contest between dashboard and subordinate reads, verifies the returned data/fence remain the original snapshot, and rejects empty workflow identities without reservations.
- `tests/e2e/contest-targeting.pw.ts` exercises delayed/failed/out-of-order selection, both write paths and failed refreshes. Its full DB-backed test also requires `PGDATABASE=gpo_library_test`, matching `PG*` fixture connection settings and `USE_REAL_LLM=false`; it fingerprints all application tables after every stale/missing DELETE, checks all framed pages and scoped async reads, and verifies valid current deletes still work. Only synthetic disposable data is permitted. CI runs the snapshot and controlled-network selection tests; the full fingerprint browser suite needs a matching separately provisioned DB connection.
