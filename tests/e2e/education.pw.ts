...
test("one-editor team rehearsal: baseline, shared practice, frozen final, reveal and debrief", async ({ page, context }) => {
  ...
  await expect(page.getByRole("heading",{name:"Contest Library"})).toBeVisible();
  let state = await (await context.request.get("/api/admin/contest-schema")).json();
  const fork = await context.request.post("/api/admin/contest-schema",{headers,data:{action:"fork",contestId:state.contestId,expectedVersion:state.schema.version,expectedRevision:state.revision}});
  expect(fork.ok()).toBe(true);
  const forkId=(await fork.json()).contestId;
  state = await (await context.request.get(`/api/admin/contest-schema?contestId=${forkId}`)).json();
  Object.assign(headers,{"X-Contest-Id":state.contestId,"X-Contest-Version":String(state.schema.version)});
  ...
  expect((await context.request.post("/api/admin/contest-schema",{headers,data:{action:"schema",contestId:state.contestId,expectedVersion:state.schema.version,expectedRevision:state.revision,schema}})).ok()).toBe(true);
  state = await (await context.request.get(`/api/admin/contest-schema?contestId=${forkId}`)).json();
  Object.assign(headers,{"X-Contest-Id":state.contestId,"X-Contest-Version":String(state.schema.version)});
  ...
  expect((await context.request.post("/api/admin/contest-schema",{headers,data:{action:"answers",contestId:state.contestId,expectedVersion:state.schema.version,expectedRevision:state.revision,answers}})).ok()).toBe(true);
  state = await (await context.request.get(`/api/admin/contest-schema?contestId=${forkId}`)).json();
  expect((await context.request.post("/api/admin/contests",{headers,data:{action:"activate",contestId:state.contestId,expectedVersion:state.schema.version,expectedRevision:state.revision}})).ok()).toBe(true);
  ...
  const submit = (kind:string,prompt:string,key:string)=>context.request.post(`/api/submissions/${kind}`,{headers:{...headers,"Idempotency-Key":key},data:{participantCode,participantToken,prompt,contestId:state.contestId,schemaVersion:state.schema.version}});
