-- Reject legacy activation of custom contests without mutating any state.
...
begin
  ...
  -- Serialize with custom editor/admission on the same challenge row. The legacy
  -- RPC cannot replace a persisted custom definition or its readiness contract.
  if exists (select 1 from challenges where id = active_challenge_id and contest_schema is not null) then
    raise exception 'Custom contests must use the contest schema editor; legacy activation is not permitted.' using errcode = '55000';
  end if;
